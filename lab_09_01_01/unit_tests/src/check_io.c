...
START_TEST(print_item_title)
{
    ...
    char test[] = "Titanic";
    if (f)
        ck_assert_int_eq(print_item(movie, test, 3, TITLE_MODE), 0);
    clear_array(movie, 3);
    fclose(f);
}
END_TEST

START_TEST(print_item_year)
{
    FILE *f = fopen("./unit_tests/tests/test_01.txt", "r");
    movie_struct *movie = malloc(sizeof(movie_struct) * 3);
    ...
    char test[] = "1980";
    if (f)
        ck_assert_int_eq(print_item(movie, test, 3, YEAR_MODE), 0);
    clear_array(movie, 3);
    fclose(f);
}
END_TEST

START_TEST(print_item_author)
{
    FILE *f = fopen("./unit_tests/tests/test_01.txt", "r");
    movie_struct *movie = malloc(sizeof(movie_struct) * 3);
    ...
    if (f)
        ck_assert_int_eq(print_item(movie, test, 3, AUTHOR_MODE), 0);
    clear_array(movie, 3);
    fclose(f);
}
END_TEST

START_TEST(print_item_notfound)
{
    FILE *f = fopen("./unit_tests/tests/test_01.txt", "r");
    movie_struct *movie = malloc(sizeof(movie_struct) * 3);
    char test[] = "Robby";
    read_items(f, movie, AUTHOR_MODE);
    if (f)
        ck_assert_int_eq(print_item(movie, test, 3, AUTHOR_MODE), NOT_FOUND);
    clear_array(movie, 3);
    fclose(f);
}
END_TEST
...
Suite *io_suite(void)
{
    ...
    tcase_add_test(tc, print_item_author);
    tcase_add_test(tc, print_item_title);
    tcase_add_test(tc, print_item_year);
    tcase_add_test(tc, print_item_notfound);
